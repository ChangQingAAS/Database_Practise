...
def search_goods(keyword, pages):
    ...
    for page in range(pages):
        ...
        search_req = sess.get(url=search_url,
        ...
        search_more_req = sess.get(url=search_more_url,
                                   params=search_more_params,
                                   headers=search_more_headers,
                                   verify=False)
...
def goods_images(goods_url):
    ...
    comment_req = requests.get(url=comment_url,
    ...
    for page in range(1, pages + 1):
        ...
        club_req = requests.get(url=club_url,
                                params=club_params,
...
def download_image(path, image_url):
    ...
    size = 0
    with closing(requests.get(image_url, headers=download_headers,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n',
                        '--num',
                        help=('the number of goods to download images'),
                        type=int,
                        default=1)

...
class Calendar:
    def __init__(s, point=None):
        ...
        if point:
            ...
            x, y = (s.master.winfo_screenwidth() -
                    width) / 2, (s.master.winfo_screenheight() - height) / 2
    ...
    def __place_widgets(s):
        ...
        tk.Label(hframe, text='月', justify='left').grid(in_=hframe,
                                                        ...
                                                        row=0)
        # 日历部件
        s._calendar = ttk.Treeview(gframe,
                                   show='',
                                   selectmode='none',
                                   height=7)
        s._calendar.pack(expand=1, fill='both', side='bottom', padx=5)
        ttk.Button(bframe, text="确 定", width=6,
                   command=lambda: s._exit(True)).grid(row=0,
                                                       column=0,
                                                       sticky='ns',
                                                       padx=20)
        ttk.Button(bframe, text="取 消", width=6,
                   command=s._exit).grid(row=0, column=1, sticky='ne', padx=20)
        tk.Frame(s.G_Frame, bg='#565656').place(x=0,
                                                y=0,
                                                relx=0,
                                                rely=0,
                                                relwidth=1,
    ...
    def __setup_selection(s, sel_bg, sel_fg):
        ...
        s._font = tkFont.Font()
        s._canvas = canvas = tk.Canvas(s._calendar,
                                       background=sel_bg,
                                       borderwidth=0,
    ...
    def _pressed(s, evt=None, item=None, column=None, widget=None):
        ...
        if not bbox:  # 日历尚不可见
            s.master.after(
                20,
                lambda: s._pressed(item=item, column=column, widget=widget))
            return
        text = '%02d' % text
    ...
    def _update(s, event=None, key=None):
        ...
        if year == datetime.now().year and month == datetime.now().month:
            ...
            for _item, day_list in enumerate(
                    s._cal.monthdayscalendar(year, month)):
                if day in day_list:
                    item = 'I00' + str(_item + 2)
                    column = '#' + str(day_list.index(day) + 1)
                    s.master.after(
                        100, lambda: s._pressed(
